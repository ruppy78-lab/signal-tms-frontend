...
import {
  ...
  StatusBadge, Spinner, EmptyState, Currency, DocUploadModal, DocList,
} from '../../components/common';
import { Plus, Search, Edit, X, RefreshCw, Copy, Upload, FileText, DollarSign, Mail, Truck, Trash2, Printer, ChevronDown } from 'lucide-react';

// ─── Constants ────────────────────────────────────────────────────────────────
const CARRIER_NAME = 'Signal Transportation Ltd';
const STATUSES = ['pending','dispatched','en_route_pickup','at_pickup','in_transit','at_delivery','delivered','cancelled'];
const STATUS_LABELS = { pending:'Available', dispatched:'Dispatched', en_route_pickup:'En Route to Pickup', at_pickup:'At Pickup', in_transit:'In Transit', at_delivery:'At Delivery', delivered:'Delivered', cancelled:'Cancelled' };
...
// ─── Load Modal ───────────────────────────────────────────────────────────────
function LoadModal({ open, editing, onClose, customers }) {
  ...
  const { data: locationsData } = useQuery({
    ...
    queryFn: () => api.get('/locations', { params:{ limit:500 } }).then(r => r.data.data || []),
  });
  ...
  const extractLC = async () => {
    ...
    try {
      ...
      const r = await api.post('/lc/extract', fd, { headers:{'Content-Type':'multipart/form-data'} });
      const d = r.data.data;
      setLcResult(d);
      ...
      toast.success('Rate confirmation extracted! Review and save the load.');
    } catch (err) {
  ...
  return (
    <>
      ...
      <div style={{
        ...
        width:'96%', maxWidth:1220, maxHeight:'96vh',
        ...
        <div style={{ display:'flex',borderBottom:'2px solid #dde3ea',background:'#f0f4f8',flexShrink:0 }}>
          {[['main','Load Details'],['docs','Documents'],['activity','Activity']].map(([id,label])=>(
            <button key={id} onClick={()=>setTab(id)} style={{ padding:'8px 20px',border:'none',background:'transparent',fontSize:13,fontWeight:tab===id?700:400,color:tab===id?'#003865':'#666',borderBottom:tab===id?'2px solid #003865':'2px solid transparent',cursor:'pointer' }}>{label}</button>
          ))}
        </div>
        ...
        <div style={{ flex:1,overflowY:'auto' }}>
                  ...
                  <table style={{ width:'100%',borderCollapse:'collapse' }}><tbody>
                    <tr>
                      ...
                      <td style={{ paddingBottom:6 }}>
                        <select style={{ width:'100%',padding:'5px 8px',border:'1px solid #ccc',borderRadius:3,fontSize:13,fontWeight:600,color:'#003865' }} value={form.customer_id} onChange={e=>setField('customer_id',e.target.value)}>
          ...
          {tab==='docs'     && <div style={{ padding:16 }}>{editing?<DocList entityType="load" entityId={editing.id} entityLabel={editing.load_number}/>:<p style={{ color:'#999',fontSize:13 }}>Save the load first to upload documents.</p>}</div>}
...
export default function Loads() {
  ...
  const [showCols,setShowCols] = useState({
    ...
    status:true, reference:false, miles:false,
  });
  ...
  const rowCtx = useCallback((e,l)=>openMenu(e,[
    ...
    {label:'Duplicate load',          icon:Copy,       action:()=>{setEditing(null);setModal(true);}},
    {divider:true},
  ...
  return (
    <div>
      ...
      <div style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:8,padding:'10px 12px',marginBottom:10}}>
        ...
        <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
          ...
          <div style={{display:'flex',gap:4,flexWrap:'wrap'}}>
            {[['','All'],['pending','Available'],['dispatched','Dispatched'],['in_transit','In Transit'],['delivered','Delivered'],['cancelled','Cancelled']].map(([val,label])=>(
            ...
            <tbody>
              {rows.map(l=>{
                ...
                <tr key={l.id} onContextMenu={e=>rowCtx(e,l)} onDoubleClick={()=>openModal(l)}
                  ...
                  {/* Status — always */}
                  <td style={{padding:'5px 8px'}}><StatusBadge status={l.status}/></td>
                </tr>
                );
              })}
            </tbody>
      ...
      <DocUploadModal open={!!docUpload} onClose={()=>setDocUpload(null)} entityType="load" entityId={docUpload?.id} entityLabel={docUpload?.label} defaultDocType={docUpload?.docType||'other'}/>
