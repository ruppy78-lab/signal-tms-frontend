...
import { ContextMenu, Pagination, Spinner, Confirm, Modal, DocUpload } from '../../shared/components';
import { Plus, Search, RefreshCw, Printer, Mail, DollarSign, X, Edit, Copy, Eye, Upload, Trash2, FileText } from 'lucide-react';

// ─── Inline helpers (v2 components not in v3) ────────────────────────────────
...
function Field({ label, required, children, className, style }) {
  return <div className={className} style={style}><label className="form-label">{label}{required && ' *'}</label>{children}</div>;
}

// ─── Constants ────────────────────────────────────────────────────────────────
const STATUS_COLOR = { draft:'#888', sent:'#0063A3', unpaid:'#E65100', partial:'#1D4ED8', paid:'#2E7D32', overdue:'#B71C1C', void:'#aaa' };
const STATUS_BG    = { draft:'#f5f5f5', sent:'#E3F2FD', unpaid:'#FFF3E0', partial:'#DBEAFE', paid:'#E8F5E9', overdue:'#FFEBEE', void:'#f5f5f5' };
...
async function printInvoice(inv, loadDocs=[], invoiceDocs=[]) {
  ...
  const chargeLines = lines.filter(l => l.line_type !== 'commodity');
  const commLines   = lines.filter(l => l.line_type === 'commodity');
  ...
  <div class="header">
    <div>
      ...
      <div style="font-size:10px;color:#555;margin-top:4px;line-height:1.7">3170 194th St Unit 102, Surrey, BC V3Z 0N4<br/>Tel: 778-663-5001<br/>signaltrucking@gmail.com</div>
    </div>
    <div>
  ...
  <div class="top-grid">
    <div class="bill-box">
      ...
      ${inv.customer_address?`<div style="margin-top:2px">${inv.customer_address}</div>`:''}
      <div>${[inv.customer_city,inv.customer_province,inv.customer_postal].filter(Boolean).join(', ')}</div>
      ${inv.customer_phone?`<div style="margin-top:2px">Tel: ${inv.customer_phone}</div>`:''}
      ${inv.customer_email?`<div style="color:#0063A3">${inv.customer_email}</div>`:''}
    </div>
    <div style="font-size:11px;line-height:2;padding:8px 0">
      ${(inv.customer_ref||inv.ref_number||inv.po_number)?`<div><b>Ref #:</b> ${inv.customer_ref||inv.ref_number||inv.po_number}</div>`:''}
      ${inv.equipment_type?`<div><b>Equipment:</b> ${inv.equipment_type}</div>`:''}
      ${inv.pickup_date?`<div><b>Pickup:</b> ${new Date(inv.pickup_date).toLocaleDateString()}</div>`:''}
    </div>
  </div>
  ...
  <table>
    <thead><tr>
      <th style="width:40px"></th><th>Company</th><th>Address</th><th>City / State</th><th style="width:80px">Reference</th>
    </tr></thead>
    <tbody>
      <tr>
        <td><span class="stop-type" style="color:#003865">PU</span></td>
        <td>${inv.shipper_name||'—'}</td><td>${inv.shipper_address||''}</td>
        <td>${[inv.shipper_city,inv.shipper_province,inv.shipper_postal].filter(Boolean).join(', ')||'—'}</td><td></td>
      </tr>
      <tr>
        <td><span class="stop-type" style="color:#059669">DEL</span></td>
        <td>${inv.consignee_name||'—'}</td><td>${inv.consignee_address||''}</td>
        <td>${[inv.consignee_city,inv.consignee_province,inv.consignee_postal].filter(Boolean).join(', ')||'—'}</td><td></td>
      </tr>
    </tbody>
  </table>
  ...
  <table>
    <thead><tr>
      <th>Commodity</th><th>Description</th><th>Reference</th><th class="r">Pcs</th><th class="r">Bill Wgt</th>
    </tr></thead>
    <tbody>
      ${inv.commodity ? `<tr><td>${inv.commodity}</td><td></td><td></td><td class="r">${inv.pieces||inv.total_pieces||'—'}</td><td class="r">${(inv.weight||inv.total_weight) ? Number(inv.weight||inv.total_weight).toLocaleString()+' lb' : '—'}</td></tr>` : '<tr><td colspan="5" style="color:#aaa;text-align:center;padding:8px">No commodity info</td></tr>'}
    </tbody>
  </table>
  ...
  <table>
    <thead><tr>
      <th>Accessorial</th><th>Reference</th><th class="r">Rate Ea</th><th class="r">Units</th><th class="r">Amount</th>
    </tr></thead>
    <tbody>
      ${accLines.map(l=>`<tr>
        <td>${l.description}</td><td></td>
...
function SendEmailModal({ open, inv, loadDocs, invoiceDocs, onClose, onSent }) {
  ...
  useEffect(() => {
    if (open && inv) {
      setTo(inv.customer_email || inv.billing_email || inv.main_email || '');
      setCc('signaltrucking@gmail.com');
      setSubj(`Invoice ${inv.invoice_number} — Signal Transportation Ltd`);
      const fmtAmt = '$' + parseFloat(inv.total_amount || 0).toFixed(2);
      const fmtDue = inv.due_date ? new Date(inv.due_date).toLocaleDateString('en-CA', {year:'numeric',month:'short',day:'numeric'}) : '—';
      const fmtInvDate = inv.created_at ? new Date(inv.created_at).toLocaleDateString('en-CA', {year:'numeric',month:'short',day:'numeric'}) : '—';
      setBody(`Dear ${inv.customer_name || 'Valued Customer'},

Please find attached Invoice ${inv.invoice_number} for shipment ${inv.load_number || ''}.

INVOICE DETAILS:
─────────────────────────
${(inv.customer_ref||inv.ref_number||inv.po_number) ? 'Ref #:         ' + (inv.customer_ref||inv.ref_number||inv.po_number) + '\n' : ''}Invoice Date:  ${fmtInvDate}
Due Date:      ${fmtDue}
Amount Due:    ${fmtAmt}

SHIPMENT:
Pickup:    ${inv.shipper_name || '—'} — ${inv.shipper_city || ''}
Delivery:  ${inv.consignee_name || '—'} — ${inv.consignee_city || ''}
Commodity: ${inv.commodity || '—'}
Pieces:    ${inv.pieces || inv.total_pieces || '—'}
Weight:    ${(inv.weight || inv.total_weight) ? Number(inv.weight || inv.total_weight).toLocaleString() + ' lb' : '—'}
─────────────────────────
...
Please remit payment by ${fmtDue}.

Thank you for your business.

Signal Transportation Ltd
3170 194th St Unit 102
Surrey BC V3Z 0N4
Tel: 778-663-5001
signaltrucking@gmail.com`);
    }
  }, [open, inv]);

  const sendMut = useMutation({
    mutationFn: () => api.put(`/invoicing/${inv.id}/send`),
    onSuccess: () => { toast.success('Invoice marked as sent'); qc.invalidateQueries({queryKey:['invoices']}); qc.invalidateQueries({queryKey:['invoice-detail']}); onSent(); onClose(); },
...
function EditInvoiceModal({ inv, onClose, onSaved }) {
  ...
  useEffect(() => {
    if (inv) {
      ...
      api.get(`/invoicing/${inv.id}`).then(r => {
        const items = r.data?.line_items || [];
  ...
  const saveMut = useMutation({
    mutationFn: () => api.put(`/invoicing/${inv.id}`, {
    ...
    onSuccess: () => { toast.success('Invoice updated'); qc.invalidateQueries({queryKey:['invoices']}); qc.invalidateQueries({queryKey:['invoice']}); onSaved(); },
  ...
  const TH2 = {fontSize:10,fontWeight:700,padding:'5px 8px',background:'#003865',color:'#fff',textAlign:'left'};
  const TD2 = {padding:'3px 4px',border:'1px solid #eee'};
            ...
            <tbody>
              {lines.map((l,i)=>(
                <tr key={i} style={{background:i%2===0?'#fff':'#fafafa'}}>
                  ...
                  <td style={{...TD2,width:100,fontWeight:700,textAlign:'right',padding:'4px 8px'}}>
                    ${(parseFloat(l.amount)||0).toFixed(2)}
                  </td>
                  <td style={{...TD2,width:28,textAlign:'center'}}>
                    {lines.length>1&&(
...
function InvoiceDetail({ invoiceId, onClose, onRefresh }) {
  ...
  const { data: invData, isLoading } = useQuery({
    ...
    queryFn: () => api.get(`/invoicing/${invoiceId}`).then(r => r.data),
    enabled: !!invoiceId,
  });

  const { data: loadDocs } = useQuery({
    queryKey: ['load-docs', invData?.load_id],
    queryFn: () => api.get(`/documents/load/${invData?.load_id}`).then(r => r.data || []),
    enabled: !!invData?.load_id,
  });

  const { data: invoiceDocs, refetch: refetchDocs } = useQuery({
    queryKey: ['invoice-docs', invoiceId],
    queryFn: () => api.get(`/documents/invoice/${invoiceId}`).then(r => r.data || []),
    enabled: !!invoiceId,
  });

  const payMut = useMutation({
    mutationFn: b => api.post(`/invoicing/${invoiceId}/payment`, b),
    onSuccess: () => { toast.success('Payment recorded'); qc.invalidateQueries({queryKey:['invoices']}); qc.invalidateQueries({queryKey:['invoice-detail']}); setPayModal(false); setPayForm({amount:'',payment_method:'EFT',reference:'',notes:''}); },
    onError: e => toast.error(e.response?.data?.message||'Failed'),
  });
  const voidMut = useMutation({
    mutationFn: () => api.put(`/invoicing/${invoiceId}/void`),
    onSuccess: () => { toast.success('Invoice voided'); qc.invalidateQueries({queryKey:['invoices']}); qc.invalidateQueries({queryKey:['invoice-detail']}); setConfirmVoid(false); },
    onError: e => toast.error(e.response?.data?.message||'Failed'),
  });
  const deleteMut = useMutation({
    mutationFn: () => api.delete(`/invoicing/${invoiceId}`),
    onSuccess: () => {
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%',background:'#fff'}}>
      ...
      <div style={{display:'flex',gap:6,padding:'8px 14px',borderBottom:'1px solid #eee',flexShrink:0,flexWrap:'wrap',background:'#f8f9fa'}}>
        ...
        {inv.status!=='paid'&&inv.status!=='void'&&(
          <button onClick={()=>{
            if (!inv.pod) { toast.error('POD required before sending invoice. Please upload the signed POD in the Documents section first.'); return; }
            setSendModal(true);
          }}
      ...
      <div style={{flex:1,overflowY:'auto',padding:'14px'}}>
        ...
        {/* Shipment details */}
        {(inv.shipper_name||inv.consignee_name||inv.shipper_city||inv.consignee_city)&&(
          <div style={{background:'#f0f4f8',border:'1px solid #dde3ea',borderRadius:4,padding:'8px 12px',marginBottom:12,fontSize:11}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:6}}>
              <div>
                <div style={{fontWeight:700,color:'#003865',fontSize:10,textTransform:'uppercase',marginBottom:2}}>Shipper</div>
                <div>{inv.shipper_name||'—'}</div>
                {inv.shipper_address&&<div style={{color:'#6B7280'}}>{inv.shipper_address}</div>}
                <div style={{color:'#6B7280'}}>{inv.shipper_city}{inv.shipper_province ? ', '+inv.shipper_province : ''} {inv.shipper_postal||''}</div>
                <div style={{marginTop:2}}><b>Pickup:</b> {fmtD(inv.pickup_date)}</div>
              </div>
              <div>
                <div style={{fontWeight:700,color:'#003865',fontSize:10,textTransform:'uppercase',marginBottom:2}}>Consignee</div>
                <div>{inv.consignee_name||'—'}</div>
                {inv.consignee_address&&<div style={{color:'#6B7280'}}>{inv.consignee_address}</div>}
                <div style={{color:'#6B7280'}}>{inv.consignee_city}{inv.consignee_province ? ', '+inv.consignee_province : ''} {inv.consignee_postal||''}</div>
                <div style={{marginTop:2}}><b>Delivery:</b> {fmtD(inv.delivery_date)}</div>
              </div>
            </div>
            <div style={{marginTop:6,display:'flex',gap:16,flexWrap:'wrap'}}>
              {inv.commodity&&<span><b>Commodity:</b> {inv.commodity}</span>}
              <span><b>Pcs:</b> {inv.pieces||inv.total_pieces||'—'}</span>
              <span><b>Wgt:</b> {(inv.weight||inv.total_weight) ? Number(inv.weight||inv.total_weight).toLocaleString()+' lb' : '—'}</span>
              {inv.equipment_type&&<span><b>Equipment:</b> {inv.equipment_type}</span>}
              {(inv.customer_ref||inv.ref_number||inv.po_number)&&<span><b>Ref #:</b> {inv.customer_ref||inv.ref_number||inv.po_number}</span>}
            </div>
          </div>
        )}
        ...
        <div style={{marginBottom:12}}>
          ...
          {inv.pod ? (
            <div style={{padding:'6px 10px',background:'#E8F5E9',border:'1px solid #A5D6A7',borderRadius:4,fontSize:11,color:'#2E7D32',fontWeight:600,marginBottom:8}}>
              POD uploaded — ready to send invoice
            </div>
          ) : (
            <div style={{padding:'6px 10px',background:'#FFF3E0',border:'1px solid #FFCC80',borderRadius:4,fontSize:11,color:'#E65100',fontWeight:600,marginBottom:8}}>
              No POD — required before sending invoice
            </div>
          )}
          {totalDocCount===0&&(
            <div style={{textAlign:'center',padding:'20px',color:'#aaa',border:'1px dashed #ddd',borderRadius:4,fontSize:12}}>
              ...
              <span style={{fontSize:11}}>Upload POD or Rate Confirmation to attach to invoice.</span>
            </div>
          )}
        </div>
      ...
      <Modal open={payModal} onClose={()=>setPayModal(false)} title={`Record Payment — ${inv.invoice_number}`} size="sm">
        <div className="form-grid">
          ...
          <div className="full" style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:8}}>
      ...
      {/* Doc Upload Modal */}
      <Modal open={docUpload} onClose={()=>{setDocUpload(false);refetchDocs();}} title={`Attach Document — ${inv.invoice_number}`} size="sm">
        <DocUpload entityType="load" entityId={inv.load_id} docTypes={['POD','Rate Confirmation']} onUploaded={()=>{setDocUpload(false);refetchDocs();}}/>
      </Modal>

      <Confirm open={confirmVoid} onClose={()=>setConfirmVoid(false)} onConfirm={()=>voidMut.mutate()}
        title="Void Invoice" message={`Void invoice ${inv.invoice_number}? This cannot be undone.`} confirmText="Void" variant="danger"/>

      <Confirm open={confirmDelete} onClose={()=>setConfirmDelete(false)} onConfirm={()=>deleteMut.mutate()}
        title="Delete Invoice" message={`Delete invoice ${inv.invoice_number}? Load ${inv.load_number||''} will return to ready for invoicing.`} confirmText="Delete" variant="danger"/>
      ...
      {editModal && inv && (
        <EditInvoiceModal
          ...
          onSaved={()=>{ setEditModal(false); qc.invalidateQueries({queryKey:['invoice-detail',invoiceId]}); }}
        />
      )}
    </div>
  );
...
function ReadyToBillTab({ customers }) {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const invsRes = await api.get('/invoicing', { params: { limit: 500 } });
      const invoicedLoadIds = new Set((invsRes.data || []).map(i => i.load_id).filter(Boolean));
  ...
  const createOne = async (load) => {
    setCreating(load.id);
    try {
      await api.post('/invoicing', { load_id: load.id });
      toast.success(`Invoice created for ${load.load_number}`);
      qc.invalidateQueries({queryKey:['invoices']});
      qc.invalidateQueries({queryKey:['ready-to-bill']});
    } catch (e) {
                ...
                return (
                  <tr key={load.id}
                    ...
                    onContextMenu={e=>{e.preventDefault();openRtbMenu(e,[
                      {label:'Open Load Details', icon:FileText, onClick:()=>navigate('/loads?load='+load.load_number)},
                      {label:'Create Invoice',    icon:Plus,     onClick:()=>createOne(load)},
                      {divider:true},
                      {label:'Copy Load #',       icon:Copy,     onClick:()=>{navigator.clipboard.writeText(load.load_number);toast.success('Copied!');}},
                    ]);}}
...
export default function Invoicing() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get('/invoicing',{params:{page,limit:30,search,status,from,to}}).then(r=>r),
    placeholderData: keepPreviousData,
  });
  const { data: customers } = useQuery({
    queryKey:['customers-dropdown'],
    queryFn:()=>api.get('/customers',{params:{limit:200}}).then(r=>r.data||r.rows||[]),
  });
  ...
  const syncDelivered = async () => {
    try {
      ...
      const invs  = await api.get('/invoicing', { params:{ limit:200 } });
      const loadIds = (invs.data||[]).map(i=>i.load_id).filter(Boolean);
      const missing = (loads.data||[]).filter(l => !loadIds.includes(l.id));
  ...
  const rowCtx = useCallback((e,inv)=>{
    openMenu(e,[
      {label:'View / Edit',         icon:Eye,       onClick:()=>setSelected(inv.id)},
      {label:'Print Invoice',       icon:Printer,   onClick:()=>{setSelected(inv.id);}},
      {label:'Send to Customer',    icon:Mail,       onClick:()=>api.put(`/invoicing/${inv.id}/send`).then(()=>{toast.success('Marked as sent');qc.invalidateQueries({queryKey:['invoices']});})},
      {divider:true},
      {label:'Record Payment',      icon:DollarSign, onClick:()=>setSelected(inv.id)},
      {divider:true},
      {label:'Copy Invoice #',      icon:Copy,       onClick:()=>{navigator.clipboard.writeText(inv.invoice_number);toast.success('Copied!');}},
      {label:'Open Load Details',   icon:FileText,   onClick:()=>{ if(inv.load_number){ navigate('/loads?load='+inv.load_number); } else { navigate('/loads'); } }},
      {divider:true},
      {label:'Void Invoice',        icon:X,          danger:true, onClick:()=>{if(window.confirm(`Void ${inv.invoice_number}?`))api.put(`/invoicing/${inv.id}/void`).then(()=>{toast.success('Voided');qc.invalidateQueries({queryKey:['invoices']});});}},
    ]);
  },[openMenu, navigate, qc]);
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'calc(100vh - 90px)'}}>
      ...
      <div style={{display:'flex',gap:8,marginBottom:10,flexShrink:0,flexWrap:'wrap',alignItems:'center'}}>
        ...
        <button className="btn btn-secondary btn-sm" onClick={()=>qc.invalidateQueries({queryKey:['invoices']})}><RefreshCw size={13}/></button>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:selected?'1fr 420px':'1fr',gap:10,flex:1,overflow:'hidden',minHeight:0,transition:'grid-template-columns 0.2s'}}>
        ...
        <div style={{border:'1px solid #ddd',borderRadius:6,overflow:'hidden',display:'flex',flexDirection:'column'}}>
          ...
          {paging && <Pagination page={paging.page||1} pages={paging.pages||1} total={paging.total||0}
            onPrev={()=>setPage(p=>Math.max(1,p-1))} onNext={()=>setPage(p=>p+1)} onGoTo={setPage}/>}
        </div>

        {/* Detail panel */}
        {selected && (
          <div style={{border:'1px solid #ddd',borderRadius:6,overflow:'hidden',display:'flex',flexDirection:'column'}}>
            <InvoiceDetail invoiceId={selected} onClose={()=>setSelected(null)} onRefresh={()=>qc.invalidateQueries({queryKey:['invoices']})}/>
          </div>
        )}
      </div>
...
function CreateInvoiceModal({ open, onClose, customers }) {
  ...
  const { data: loads } = useQuery({
    ...
    queryFn: () => api.get('/loads',{params:{status:'delivered',limit:100}}).then(r=>r.data||r.rows||[]),
    enabled: open,
  });
  ...
  const mut = useMutation({
    mutationFn: ()=>api.post('/invoicing',{customer_id:customerId,load_id:loadId||undefined,line_items:lines,payment_terms:terms,gst_rate:gstRate,notes}),
    onSuccess: ()=>{toast.success('Invoice created');qc.invalidateQueries({queryKey:['invoices']});onClose();},
    onError: e=>toast.error(e.response?.data?.message||'Failed'),
  });
