...
export default function AvailableLoadsPanel({ loads = [], onRefresh, onCreateTrip, loadActions, selectedIds = [], onToggleSelect, onStatusChange, onClickLoad }) {
  const { ctx, open, close } = useLoadContextMenu();

  return (
    <div className="dp-panel available" onContextMenu={e => e.preventDefault()}>
      ...
      <div className="dp-panel-body">
        ...
        {loads.length === 0 && <div style={{ padding: 20, textAlign: 'center', color: '#94a3b8', fontSize: 11 }}>No available loads</div>}
      </div>
